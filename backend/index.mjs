...
// app.use(cors({
//     origin: ['https://study-notes-sharing-app-backend.onrender.com'], // लोकल और डिप्लॉयमेंट दोनों को अनुमति दें
...
// CORS Middleware
app.use(cors({
    origin: ['http://localhost:5173', 'https://study-notes-sharing-app-frontend.onrender.com'], // Allow both localhost and deployed frontend
    methods: ['GET', 'POST', 'PUT', 'DELETE'],
    credentials: true,
}));
...
app.post("/signup", async (req, res) => {
    try {
        ...
        const data = { user: { id: user._id, role: user.role } };
        const token = jwt.sign(data, process.env.JWT_SECRET);
